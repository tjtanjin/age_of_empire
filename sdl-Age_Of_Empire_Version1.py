...
class Empire():
    def __init__(self, name):
        ...
        self.soldier = {'num': 30, 'attack': 5, 'desc': "A light infantry unit with weak attack"}
        self.warrior = {'num': 10, 'attack': 10, 'desc': "A toughened soldier with moderate attack"}
        self.knight = {'num': 5, 'attack': 20, 'desc': "A heavily armored unit with strong attack"}
def end():
    while True:
...
def playerturn():
    ...
    while True:
        ...
        #Player Hire Option
        elif player_choice == "3" or player_choice == "H" and player.gold['amount'] >= 20:
            while True:
                print("Which unit do you wish to hire?\n1. Soldier (Cost: 20 Gold | Attack: 5 | Armor: 5)\n2. Warrior (Cost: 50 Gold | Attack: 10 | Armor: 7)\n3. Knight (Cost: 100 Gold | Attack: 20 | Armor: 10)")
                player_hire = input("> ")
                if (player_hire == "1" or player_hire == "S") and player.gold['amount'] >= 20:
                    while True:
                        print("How many Soldiers do you want to hire?")
                        s_hire = input("> ")
                        if s_hire.isdigit() and int(s_hire) > 0 and int(s_hire)*20 <= player.gold['amount']:
                            break
                        else:
                            print("Invalid Option or Insufficient Gold! Please choose again!")
                    player.soldier['num'] = player.soldier['num'] + int(s_hire)
                    player.gold['amount'] = player.gold['amount'] - int(s_hire)*20
                    print("Hiring Soldiers...")
                    time.sleep(5)
                    print("%r Soldiers hired!" % int(s_hire))
                    break
                elif (player_hire == "2" or player_hire == "W") and player.gold['amount'] >= 50:
                    while True:
                        print("How many Warriors do you want to hire?")
                        w_hire = input("> ")
                        if w_hire.isdigit() and int(w_hire) > 0 and int(w_hire)*50 <= player.gold['amount']:
                            break
                        else:
                            print("Invalid Option or Insufficient Gold! Please choose again!")
                    player.warrior['num'] = player.warrior['num'] + int(w_hire)
                    player.gold['amount'] = player.gold['amount'] - int(w_hire)*50
                    print("Hiring Warriors...")
                    time.sleep(5)
                    print("%r Warriors hired!" % int(w_hire))
                    break
                elif (player_hire == "3" or player_hire == "300") and player.gold['amount'] >= 100:
                    while True:
                        print("How many Knights do you want to hire?")
                        k_hire = input("> ")
                        if k_hire.isdigit() and int(k_hire) > 0 and int(k_hire)*100 <= player.gold['amount']:
                            break
                        else:
                            print("Invalid Option or Insufficient Gold! Please choose again!")
                    player.knight['num'] = player.knight['num'] + int(k_hire)
                    player.gold['amount'] = player.gold['amount'] - int(k_hire)*100
...
def computerturn():
    ...
    time.sleep(3)
    while True:
        if computer.soldier['attack'] * computer.soldier['num'] + computer.warrior['attack'] * computer.warrior['num'] + computer.knight['attack'] * computer.knight['num'] > 300:
            strength = 4
        else:
            strength = 5
        if computer.castle_health['castle_health'] > 500:
            health = 1
        else:
            health = 0
        ...
        elif (computer_choice == 4 and computer.gold['amount'] >= 20) or (computer_choice == 5 and computer.gold['amount'] >= 20) :
            while True:
                computer_hire = random.randint(1,3)
                if computer_hire == 1 and computer.gold['amount'] >= 20:
                    while True:
                        s_hire = random.randint(1,10)
                        if int(s_hire)*20 <= computer.gold['amount']:
                            break
                        else:
                            continue
                    computer.soldier['num'] = computer.soldier['num'] + int(s_hire)
                    computer.gold['amount'] = computer.gold['amount'] - int(s_hire)*30
                    break
                if computer_hire == 2 and computer.gold['amount'] >= 50:
                    while True:
                        w_hire = random.randint(1,5)
                        if int(w_hire)*50 <= computer.gold['amount']:
                            break
                        else:
                            continue
                    computer.warrior['num'] = computer.warrior['num'] + int(w_hire)
                    computer.gold['amount'] = computer.gold['amount'] - int(w_hire)*50
                    break
                if computer_hire == 3 and computer.gold['amount'] >= 100:
                    while True:
                        k_hire = random.randint(1,3)
                        if int(k_hire)*100 <= computer.gold['amount']:
                            break
                        else:
                            continue
                    computer.knight['num'] = computer.knight['num'] + int(k_hire)
                    computer.gold['amount'] = computer.gold['amount'] - int(k_hire)*100
                    break
                else:
                    continue
